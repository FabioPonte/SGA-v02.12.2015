...
{
    class RelatorioDeEstadoDeEstoque
    {
        ...
        public string MontarHTML(DateTime data,DateTime dataAnterior, Boolean siger,Boolean ComCusto,double CustoMinimo)
        {
            html = "";
            IniciarSelecionador();
            ...
            custo_anterior = Select.SelectSQL("select distinct data, substring(POSICAO,1,4) f3, numeroitem f4,  custo f7 from condicaoestoque where data  = '" + Facilitadores.ConverterDataParaDataDoMysql.Converter(dataAnterior) + "' order by 2");
